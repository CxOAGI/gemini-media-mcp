...
# Vertex AI / Gemini Enterprise Agent Platform serves 1.1 under a different
# ID than the Gemini Developer API does — confirmed on four of its model pages
# (generate-videos-from-text / -from-an-image / -from-references, and
# extend-videos), all of which list `gemini-omni-1.1-flash-preview` and none
# of which mention the bare name. Sending the Developer-API spelling to Vertex
# reaches no model at all. src/video.py carries the same split for Veo.
#
# The preview model is spelled identically on both, so it is absent here.
_VERTEX_MODEL_IDS: dict[str, str] = {
    OMNI_1_1_MODEL: "gemini-omni-1.1-flash-preview",
}

# Inverted, so a model ID that comes BACK from Vertex resolves to the same
# spec (and the same price) instead of looking like an unknown model.
_CANONICAL_MODEL_IDS: dict[str, str] = {
    served: canonical for canonical, served in _VERTEX_MODEL_IDS.items()
}

# Every spelling this server accepts, either backend's.
OMNI_MODEL_ALIASES: tuple[str, ...] = (*OMNI_MODELS, *_VERTEX_MODEL_IDS.values())


def canonical_omni_model(model: str | None) -> str:
    """Fold a backend-specific omni ID onto the canonical one."""
    name = str(model)
...
def served_omni_model(model: str, *, vertexai: bool) -> str:
    ...
    if vertexai:
        return _VERTEX_MODEL_IDS.get(canonical, canonical)
    return canonical
...
# Tasks that continue existing footage rather than rendering something new.
_CONTINUATION_TASKS = (_TASK_EDIT, _TASK_EXTEND)


def _strips_output_spec(
    task_type: str | None, previous_interaction_id: str | None
) -> bool:
...
OMNI_SPECS: dict[str, OmniModelSpec] = {
    ...
    OMNI_1_1_MODEL: OmniModelSpec(
        ...
        # The Interactions API reference lists duration among
        # VideoResponseFormat's fields ("integers between 3 and 10, followed
        # by 's'"), and Vertex's own extend-videos request sends it. An
        # earlier read of the task-guide page alone concluded otherwise and
        # warned callers off it; the reference settles it.
        duration_is_documented=True,
        ...
        # multi-turn)." That is the Gemini Developer API's figure; Vertex
        # documents 1-30s for the same operation, hence
        # omni_source_limit_seconds rather than this value alone.
...
# Vertex/GEAP states a different ceiling for the same operation: "The input
# video is limited between 1 and 30 seconds in length" on its extend-videos
# page, against the Developer API's 10s "when uploading". Different backends,
# different limits — reading one onto the other would refuse a legitimate
# 20s Vertex source.
OMNI_VERTEX_SOURCE_MAX_SECONDS = 30.0


def omni_source_limit_seconds(spec: OmniModelSpec, *, vertexai: bool) -> float:
    """Documented ceiling on an UPLOADED edit/extension source, by backend."""
    if not spec.max_uploaded_source_seconds:
        return 0.0
    if vertexai:
        return OMNI_VERTEX_SOURCE_MAX_SECONDS
    return spec.max_uploaded_source_seconds


def is_omni_model(model: str | None) -> bool:
    """Whether ``model`` is served through the Interactions API here.

    True for either backend's spelling, so a model ID reported back by Vertex
    still routes and still prices.
    """
    return canonical_omni_model(model) in OMNI_SPECS
...
def omni_spec(model: str | None) -> OmniModelSpec:
    ...
    """
    spec = OMNI_SPECS.get(canonical_omni_model(model))
    if spec is None:
        raise ValueError(
...
def _build_create_kwargs(
    *,
    ...
    if not _strips_output_spec(task_type, previous_interaction_id):
...
async def generate_video_omni(
    ...
    *,
    ...
    if _strips_output_spec(task_type, previous_interaction_id):
        # Whichever of the two reasons applies (see _strips_output_spec), the
    ...
    # Which spelling this backend serves. Vertex publishes 1.1 as
    # `gemini-omni-1.1-flash-preview`; the Developer API as the bare name.
    # Sending the wrong one reaches no model at all.
    vertexai = bool(getattr(getattr(client, "_api_client", None), "vertexai", False))
    served_model = served_omni_model(spec.model, vertexai=vertexai)
    ...
    result: dict[str, Any] = {
        ...
        # The canonical ID, so a manifest, a price lookup and a follow-up call
        # all agree regardless of which backend rendered it. What actually
        ...
        **({"requested_model": str(model)} if str(model) != spec.model else {}),
        ...
        "duration_seconds": None if withholds_output_spec else clamped_duration,
        ...
        # request's.
        "aspect_ratio": None if withholds_output_spec else aspect_ratio,
