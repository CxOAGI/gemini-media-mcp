...
async def _omni_generate_and_manifest(
    ...
    *,
    ...
    # An edit inherits its length from the source, so the impl reports None
    # rather than the request it never sent. Resolve the real length from the
    # source's sidecar — the same lookup the quote uses, so quote and bill
    # agree — and fall back honestly when the source is not ours.
    ...
    duration_is_known = effective_duration is not None
    if not duration_is_known and previous_interaction_id:
        effective_duration = await _source_duration_or_none(
            app_ctx.videos_dir, previous_interaction_id
        )
        duration_is_known = effective_duration is not None
        if duration_is_known:
            result["duration_seconds"] = effective_duration
            result["duration_source"] = "inherited from the source video"
        else:
            result["duration_source"] = (
                "unknown: this edit inherits the source video's length, and "
                "the source was not generated by this server"
            )
    ...
    cost = _video_cost(
        ...
        resolution="720p",
        include_audio=False,
        actual=duration_is_known,
    )
    if cost:
        if not duration_is_known:
            cost = dict(cost)
            cost["detail"] = (
                f"{cost['detail']} — the source video's length is unknown, so "
                "this is the requested duration, not the billed one"
            )
        result["cost"] = cost
...
async def edit_video(
    ...
    Args:
        ...
        aspect_ratio: "16:9" (default) or "9:16"
        duration_seconds: Not sent for edits — verified on the wire — because
            the API rejects duration on an edit task. The render inherits the
            source video's length, so this value does not change the output.
            It is used only as a fallback figure for the cost estimate when
            the source's real length cannot be resolved.
